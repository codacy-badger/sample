...
class Cradle_Module_Oauth_Auth_Service_SqlServiceTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testCreate()
    {
        ...
        $this->assertEquals(3, $auth['auth_id']);

        $auth = $this->object->create([
            'auth_slug' => 'test@gmail.com',
            'auth_password' => '482c811da5d5b4bc6d497ffa98491e38',
            'auth_active' => '1',
            'auth_permissions' => '["public_profile", "public_product", "personal_profile", "personal_product", "personal_comment", "personal_review", "user_profile", "user_product", "user_comment", "user_review"]',
            'auth_type' => 'admin'
        ]);

        $this->assertEquals(4, $auth['auth_id']);

        $auth = $this->object->create([
            'auth_slug' => 'testseeker123@gmail.com',
            'auth_password' => '482c811da5d5b4bc6d497ffa98491e38',
            'auth_active' => '1',
            'auth_permissions' => '["public_profile", "public_product", "personal_profile", "personal_product", "personal_comment", "personal_review", "user_profile", "user_product", "user_comment", "user_review"]',
            'auth_type' => 'admin'
        ...
        $this->assertEquals(5, $auth['auth_id']);
    }
    ...
    public function testLinkProfile()
    {
        ...
        // link profile
        $actual = $this->object->linkProfile(3, 5);

        $this->assertTrue(!empty($actual));
        $this->assertEquals(3, $actual['auth_id']);
        $this->assertEquals(5, $actual['profile_id']);

        // link profile
        $actual = $this->object->linkProfile(4, 6);

        $this->assertTrue(!empty($actual));
        $this->assertEquals(4, $actual['auth_id']);
        $this->assertEquals(6, $actual['profile_id']);
    }
