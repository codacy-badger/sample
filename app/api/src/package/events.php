...
$cradle->on('rest-permitted', function ($request, $response) {
    ...
    if ($profile) {
        ...
        if (!in_array($role, $request->get('source', 'app_permissions')) &&
            !in_array('admin_' . $role, $request->get('source', 'app_permissions')) &&
            !in_array('marketing_dashboard', $request->get('source', 'app_permissions')) &&
            !in_array('business_dashboard', $request->get('source', 'app_permissions'))) {
            return $response->setError(true, 'Unauthorize Request');
        }
    } else if ($request->get('source', 'type') === 'app') {
        //I should have a personal role, or marketing or business role
        if (!in_array('personal_' . $role, $request->get('source', 'app_permissions')) &&
            !in_array($role, $request->get('source', 'app_permissions'))) {
            return $response->setError(true, 'Unauthorize Request');
        }
...
$cradle->on('rest-app-permitted', function ($request, $response) {
    ...
    $search = $database
        ->search('app')
        ->setColumns('profile.*', 'app.*', 'auth_permissions')
    ...
    if ($row['auth_permissions']) {
        $row['auth_permissions'] = json_decode($row['auth_permissions'], true);
    } else {
        $row['auth_permissions'] = [];
    }
...
$cradle->on('rest-session-permitted', function ($request, $response) {
    ...
    $search = $database
        ->search('session')
        ->setColumns('session.*', 'profile.*', 'app.*', 'auth_permissions')
