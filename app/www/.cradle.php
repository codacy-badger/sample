<?php //-->
/**
 * This file is part of a Custom Project.
 * (c) 2017-2019 Acme Inc.
 *
 * Copyright and license information can be found at LICENSE.txt
 * distributed with this package.
 */

//include the other routes
include_once __DIR__ . '/src/controller/ajax.php';
include_once __DIR__ . '/src/controller/auth.php';
include_once __DIR__ . '/src/controller/blog.php';
include_once __DIR__ . '/src/controller/canonical.php';
include_once __DIR__ . '/src/controller/interview.php';
include_once __DIR__ . '/src/controller/post.php';
include_once __DIR__ . '/src/controller/profile.php';
include_once __DIR__ . '/src/controller/research.php';
include_once __DIR__ . '/src/controller/static.php';
include_once __DIR__ . '/src/controller/tracking.php';
include_once __DIR__ . '/src/controller/widget.php';

//include globals, events, methods
include_once __DIR__ . '/src/package/methods.php';
include_once __DIR__ . '/src/package/events.php';
include_once __DIR__ . '/src/package/globals.php';
